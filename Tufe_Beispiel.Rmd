...
header-includes:
  ...
  - \fancyfoot[LE]{\color{DoStat} \thepage \hspace{0.5cm} dortmunder\textbf{statistik}  • Tufte • eine Alternative zum 2-spaltigen Layout • ein Kurzbeispiel}
  - \fancyfoot[RO]{\color{DoStat} dortmunder\textbf{statistik}  • Tufte • eine Alternative zum 2-spaltigen Layout • ein Kurzbeispiel \hspace{0.5cm} \thepage}
...
# Einführung in das tufte package

Vorteile:

- Flexible Verwendung von Seitennotizen
- "modernes" Layout, in Abgrenzung zu bestehendem 1 bzw 2-spaltigem Layout
- support über "bookdown" package, dadurch 
  + Produktion von HTML und PDF Versionen aus einem Dokument!
  + kontinuierlicher Support über Community
  + trotzdem Einbindung von fortgeschrittenem Latex möglich
  
Das Format wird auch in Langfassungen verwendet.^[Zefir [Wege zur Metropole Ruhr](http://www.zefir.rub.de/mam/content/zefir_mat_bd_6_wege_zur_metropole_ruhr.pdf)]

Eine weitere Alternative ist das package [tint](https://www.rdocumentation.org/packages/tint/versions/0.1.3) 


```{r  Import, echo = FALSE, message=FALSE, warning = FALSE, results=FALSE}
...
DISCLAIMER: natürlich müssen sämtliche Grafiken an das neue Layout Format angepasst werden. Hier werden Grafiken aus dem Standard Latex Template verwendet.

Ein Plot mit seitlich ausgerichteter Karte. Deutlich zu sehen: hier muss einiges an Arbeit investiert werden, um die Plots und Schriften auszurichten und zu gestalten.
```{r Plot map 1 spaltig, echo = FALSE, message=FALSE, warning=FALSE, fig.margin = TRUE}
Plot_map_SB_ND
```
...
Deshalb werden im Anschluss Beispiele aus integrierten Datensets in R verwendet, um die Anwendung zu verdeutlichen.
...
Es geht auch andersherum: Plots, die viel Fläche verbrauchen, können entsprechend seitlich kommentiert werden:


```{r r fig-main, fig.cap = "Auch hier muss deutlich umformatiert werden", cache=TRUE, echo = FALSE}
# ein seitlicher Kommentar der Karte
Plot_map_SB_ND
```
...
```{r fig-main, fig.cap = "Das sieht schon besser aus", cache=TRUE,echo = FALSE}
