...
# monofont:
# mathfont:
# linkcolor: "MidnightBlue" # Vorsicht: Links sind auch in Verzeichnissen
# urlcolor: "MidnightBlue" # Vorsicht: Links sind auch in Verzeichnissen
...
header-includes:
  ...
  # einseitiges multicol Format mit unterschiedlicher Spaltenbreite     
  - \usepackage{vwcol}      
...
SB <- sf::st_read(dsn = "./shapes/SB/Stadtbezirke_DO.shp") %>% 
  mutate(
...
```{r Map, echo = FALSE, message=FALSE, warning=FALSE, sanitize=FALSE}
# Farbpalette
col_scale <- c(
      "#d6ebff",
      "#94c9ff",
...
```{r kabledata, echo = FALSE, message=FALSE, warning=FALSE}
...
\begin{table}[!bh]
  \begin{tabular}{  r  l }
    \multicolumn{2}{l}{\hspace{2cm}\boldDoStat{Impressum}} \\
    \makecell[tr]{\hspace{2cm}\textbf{Herausgeber}} &  \makecell[l]{Stadt Dortmund, 3/Dez - Stabsstelle Dortmunder Statistik, \\  44122 Dortmund, 07/2019} \\
    \makecell[tr]{\hspace{2cm}\textbf{Redaktion}} &  \makecell[l]{Berthold Haermeyer (verantwortlich), Manfred Gabriel,\\ Roland Scheebaum, Georg Schulte, Iwona Szargut } \\
    \makecell[tr]{\hspace{2cm}\textbf{Satz}} &  \makecell[l]{Rebecca Schluck} \\ 
    \makecell[tr]{\hspace{2cm}\textbf{Layout}} &  \makecell[l]{Gerd Schmedes, Gabak Solutions, Grafische Konstruktionen, Dortmund} \\
    \makecell[tr]{\hspace{2cm}\textbf{Kontakt}} &  \makecell[l]{InfoLine (0231) 50-22124, Telefax: (0231) 50-24777} \\
    \makecell[tr]{\hspace{2cm}\textbf{eMail}} &  \makecell[l]{info.statistik@stadtdo.de} \\
    \makecell[tr]{\hspace{2cm}\textbf{Internet}} &  \makecell[l]{www.statistik.dortmund.de} \\
    \multicolumn{2}{l}{\hspace{2cm}\boldDoStat{\boldDoStat{Nachdruck, auch auszugsweise, mit Quellenangabe gestattet.}}} \\
  \end{tabular}
\end{table}
...
Nicht nummerierte Sektionen (siehe Inhaltsverzeichnis) können im Titel über \{-} bestimmt werden, also hier bspw. \#Inhaltsverzeichnis {-}
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, sanitize=FALSE}
Plot_map_SB_ND
```
...
```{r echo = FALSE, fig.cap="blubb" } 
HWBVBarPlot
```



```{r echo = FALSE, fig.cap="Bevölkerungsentwicklung in Deutschland und Frankreich"} 
popLineGraph
```
