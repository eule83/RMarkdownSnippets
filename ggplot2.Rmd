...
Male <- ggplot(
  ...
  theme(
    ...
    plot.margin = unit(c(t = -0.2, r = 0.5, b = 0.5, l = -1.5), "cm")) 
...
Female <- ggplot(
  ...
  theme(
    # durch die umgekehrte Ausrichtung der weiblichen/männlichen Plots
    # müssen die margins separat gesetzt werden
    plot.margin = unit(c(t = -0.2, r = -1.5, b = 0.5, l = 0.5), "cm")) 
...
plotBarGrouped <- BarData %>% 
    ...
    geom_bar(
      ...
      stat = "identity") +
    scale_fill_manual(labels = c("männlich", "weiblich"), values = c("blue", "green")) +
